    ...
    )
{
    ...
    if (viewTransformAcquired)
    {
		...
		//bxd ×óÓÒÑÛ
		XMMATRIX traslationL = DirectX::XMMatrixTranslation(0.2f, 0.0f, 0.0f);
		XMMATRIX traslationR = DirectX::XMMatrixTranslation(-2.1f, 0.0f, 0.0f);



		XMStoreFloat4x4(
			&viewProjectionConstantBufferData.viewProjection[0],
			XMMatrixTranspose(XMLoadFloat4x4(&cameraProjectionTransform.Left) * traslationL)
		);
		XMStoreFloat4x4(
			&viewProjectionConstantBufferData.viewProjection[1],
			XMMatrixTranspose(XMLoadFloat4x4(&cameraProjectionTransform.Right) * traslationR)
		);
